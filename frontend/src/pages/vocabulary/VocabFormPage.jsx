...
export default function FormPage({ onGenerate, onBack, loading, error, prefillData }) {
  ...
  const handleFileUpload = async (e) => {
    ...
    fd.append('file', file)
    try {
      const res = await fetch('/api/vocabulary/rag/add-file', { method: 'POST', body: fd })
  ...
  const handleFetchUrl = async (url) => {
    ...
    try {
      const res = await fetch('/api/vocabulary/extract-url', {
  ...
  const handleFetchYoutube = async (url) => {
    ...
    try {
      const res = await fetch('/api/vocabulary/extract-youtube', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ url }),
        signal: controller.signal,
      })
